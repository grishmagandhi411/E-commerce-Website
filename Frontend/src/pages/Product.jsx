    ...
    <div className="border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100">
      ...
      <div className="flex gap-12 sm:gap-12 flex-col sm:flex-row">
        ...
        <div className="flex-1">
                ...
                productData.sizes.map((item, index) => (
                  <button
                    ...
                    className={`border border-gray-100 py-2 px-4 ${
                      item.size === size.size ? "border-orange-500" : ""
                    } ${
                      item.quantity === 0 ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                  >
                    {item.size}{" "}
                    {!item.quantity ? "(Out of stock)" : `(${item.quantity})`}
                  </button>
                ))
              ) : (
          ...
          <button
            onClick={() => addToCart(productData._id, size.size, size.quantity)}
      ...
      <div className="mt-20">
        ...
        <div className="flex flex-col gap-4 border px-6 py-6 border-gray-600 text-sm">
          <p>
            ...
            quibusdam consectetur quis necessitatibus deserunt molestiae odio
            odit harum accusamus libero!
          </p>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis id
            vel laudantium numquam facilis at recusandae, illo culpa.
          </p>
        </div>
      </div>
